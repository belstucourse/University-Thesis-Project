...
@Component
@AllArgsConstructor
public class SetupRoleDBConfig implements
    ...
    public void onApplicationEvent(final ContextRefreshedEvent event) {
        ...
        if (alreadySetup)
            return;
        final Authority readAuthority
                = createAuthorityIfNotFound("READ");
        final Authority writeAuthority
                = createAuthorityIfNotFound("WRITE");
        final Authority updateAuthority
                = createAuthorityIfNotFound("UPDATE");
        final Authority deleteAuthority
                = createAuthorityIfNotFound("DELETE");

        final Set<Authority> adminAuthorities = new HashSet<>(asList(
                readAuthority, writeAuthority, updateAuthority, deleteAuthority));
        final Set<Authority> clientAuthorities = new HashSet<>(asList(
                readAuthority, writeAuthority, updateAuthority));
        final Set<Authority> supportAuthorities = new HashSet<>(asList(
                readAuthority, writeAuthority, updateAuthority, deleteAuthority));
        final Set<Authority> psychologistAuthorities = new HashSet<>(asList(
                readAuthority, writeAuthority, updateAuthority));
    ...
    private Role createRoleIfNotFound(
            final String name, final Set<Authority> authorities) {
